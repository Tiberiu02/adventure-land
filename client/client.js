...
function render() {
    try {
        ...
        if (ctx.zoom > .15) {
            ctx.clear("#85e085");
            ctx.drawFloor(-me.x, -me.y);
            ctx.drawGrid(-me.x, -me.y, 100, 5, "#FFF");
        ...
        if (entity.length != 0) {
            for (var i = 0; i < entity.length; i++) {
                ...
                if (e.rightHand) {
                    var s = parseInt(e.img.slice(7)); // Assuming that the entity is a player
                    var x, y;
                    [x, y] = getItemPosRight(s);
                    ctx.drawImage2("item/" + e.rightHand + ".png", e.x - me.x + x * Math.cos(e.angle) + y * Math.sin(e.angle), e.y - me.y + x * Math.sin(e.angle) - y * Math.cos(e.angle), 32, 32, e.angle - Math.PI / 4);
                }
                if (e.leftHand) {
                    var s = parseInt(e.img.slice(7)); // Assuming that the entity is a player
                    var x, y;
                    [x, y] = getItemPosLeft(s);
                    ctx.drawImage2("item/" + e.leftHand + ".png", e.x - me.x + x * Math.cos(e.angle) + y * Math.sin(e.angle), e.y - me.y + x * Math.sin(e.angle) - y * Math.cos(e.angle), 32, 32, e.angle - Math.PI / 4);
                }
...
function connectIo() {
    ...
    */
    ...
    socket.on('disconnect', function() {
        console.log("Connection lost, refresh the page");
        socket.disconnect()
    });
